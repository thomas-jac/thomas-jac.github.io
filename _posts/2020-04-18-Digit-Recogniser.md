...
In my last post, I talked about using Keras as the programming framework for building my Convolutional Neural Network (CNN). Today, I'd like to write about my experience using PyTorch and Keras both for making a handwritten digit recognizer on [kaggle](https://www.kaggle.com/c/digit-recognizer).  

PyTorch initially seemed a bit difficult to me given that I was used to the level of abstractions in Keras where you could contruct a fully fledged NN in a couple lines with relative ease. PyTorch also seemed to be more unintuitive in its syntax and sets of functions. After going through loads of documentation and being stuck on a bad bug for 2 days, I came to realise (the hard way) that its much more dynamic and flexible than Keras.
